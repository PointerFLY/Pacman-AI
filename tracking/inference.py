...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            distribute = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            oldPossible = self.beliefs[p]

            for k, v in distribute.items():
                allPossible[k] += v * oldPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for _ in range(self.numParticles):
            idx = idx % len(self.legalPositions)
            p = self.legalPositions[idx]
            particles.append(p)
            idx += 1
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.particles:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            particleDist[p] += emissionModel[trueDistance]
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            ...
            newParticles.append(util.sample(dist))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for _ in range(self.numParticles):
            idx = idx % len(permutations)
            particles.append(permutations[idx])
            idx += 1
    ...
    def observeState(self, gameState):
        ...
        if self.numGhosts == sum([d is None for d in noisyDistances]):
            return
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    continue
                ghost = p[i]
                trueDist = util.manhattanDistance(ghost, pacmanPosition)
                odd *= emissionModels[i][trueDist]
            particleDist[p] += odd
        ...
        newParticles = []
        for i in range(self.numParticles):
            g = util.sample(particleDist)
            newParticles.append(g)
